...
if(isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])) {
	...
	$secret = '6Le-QjkbAAAAAFbg9-AJ3yxeH86PNfNqdWZIOlJy';
	...
	if($responseData->success) {
		...
		$subject = ( isset($_POST['subject']) ) ? $_POST['subject'] : 'Wiadomość z formularza kontaktowego ze strony www.podmlynem.pl';
		...
		$arrResult = array ('response'=>'error','errorMessage'=>'reCaptcha Error: Weryfikacja recaptchy nie powiodzła sie skontaktuj sie z administratorem strony.');
		echo json_encode($arrResult);
	}

} else { 
	$arrResult = array ('response'=>'error','errorMessage'=>'reCaptcha Error: Invalid token. Prosze skontaktuj sie z administratorem.');
	echo json_encode($arrResult);
}